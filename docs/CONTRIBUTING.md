...
We welcome bug reports, feature requests, and pull requests. This guide covers the process for each.
...
- **Library version** (current: `1.0`) and **.NET version**.
...
3. **Write code** following the existing conventions:
   ...
   - Ensure thread‑safety: use the internal lock correctly.
...
| Commit messages | Imperative mood, concise (`Fix race condition in writer release`) |
...
By contributing, you agree that your contributions are licensed under the [MIT License](https://github.com/Lonewolf239/AsyncReaderWriterLock/blob/main/LICENSE).
